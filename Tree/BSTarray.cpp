...
bool binary_search_recur(vector<int> &arr, int a, int b, int ele){
    ...
    if(ele < arr[m]) return binary_search_recur(arr,a,m-1,ele); //The limit that we put from a to m-1 implies that the range we need to consider.
    return binary_search_recur(arr,b,m+1,ele);

}

bool binary_search_itr(vector<int> &arr, int ele){ //For itr function we don't declare the int a and b values here but later on and specify the 0 and stuff.
    ...
    while(a<b){
        ...
        else a = m+1; //Hence, m+1 means all the number till the upper half and same casse applies to b as well
    }
    return false;
}
int main(){
